...
LOG_FILE_PATH = "/your/onprem/path/anaplan_cloudworks_log.txt"
...
def generate_payload(private_key_path, passphrase=None, payload_version="v2"):
    ...
    # === DEBUG: Private key info
    print("Debug: Private Key Info")
    print(f"Key size (bits): {private_key.size_in_bits()}")
    print(f"Modulus (n): {private_key.n}")
    print()
    ...
    print("Debug: Payload and Signature")
    print(f"Encoded data (base64): {encoded_data[:100]}...")  # Truncated for readability
    print(f"Encoded signature (base64): {encoded_signed_data[:100]}...")
    print()
...
def load_certificate_base64(cert_path):
    ...
    with open(cert_path, "rb") as cert_file:
        ...
        cert_base64 = b64encode(cert_data).decode("utf-8")

    print("Debug: Certificate Info")
    print(f"Certificate loaded. Base64 length: {len(cert_base64)} characters")
    print("Note: Modulus check skipped as 'cryptography' and 'openssl' not used.")
    print()
...
def main():
    try:
        ...
        print(f"Passphrase provided: {'Yes' if PRIVATE_KEY_PASSPHRASE else 'No'}")
