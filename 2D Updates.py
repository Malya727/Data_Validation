...
def auto_column_mapping(df1_cols, df2_cols):
    """
    Automatically maps columns from df1 to df2 using exact or alias matching.
    Alias matching ignores case and non-alphanumeric chars.
    """
    def normalize(col):
    ...
    for col1 in df1_cols:
        ...
        if norm1 in normalized_df2:
            ...
            # Try partial or fuzzy matching (can extend here)
            # For now, no match found, skip
            pass
    return mapping

# --- Data Validation: Null, Special Char, Negative Checks ---
def data_validation(df):
    ...
    # Special characters check (non-alphanumeric except space, dot, underscore)
    ...
    # Prepare detailed report for all issues
...
def cell_by_cell_comparison(df1, df2, mapping, unique_cols):
    ...
    console.print(Panel.fit(differences.head().to_string(), title="[red]Sample Differences[/red]"))
...
def row_by_row_comparison(df1, df2, mapping):
    ...
    # Create signature strings for rows (concatenate all columns)
...
def main():
    ...
    if mode == 1:
        ...
        # Auto column mapping with alias handling
